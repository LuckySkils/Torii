...
import { type DashboardRelease, type ReleaseSummary } from '@/types/subtracker';
...
interface ReleaseCardProps {
    release: ReleaseSummary | DashboardRelease;
    showLink?: boolean;
    /** Leading thumbnail (the dashboard passes the show poster). */
    poster?: ReactNode;
    /** Replaces the default episode label. */
    episode?: ReactNode;
    ...
    const details = (
        <>
            ...
            {showLink && release.show && (
                <Link href={`/shows/${release.show.id}`} className="text-sm font-medium break-words hover:underline">
                    {release.show.name}
                </Link>
            )}
